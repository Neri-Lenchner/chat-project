...
7. **TODO-6:** `otherUserId` מגיע מ-`roomMeta`. אם הוא לא ידוע (חדר מהשרת ללא meta מקומי) — שדה הכתיבה מושבת עם `.composer__error`: `לא ניתן לזהות את הנמען בשיחה הזו`.
...
| — | `otherUserId?: number` | `roomMeta` מקומי · TODO-6 |
| — | `displayName: string` | מחושב: meta ← name ← `שיחה #{id}` |
